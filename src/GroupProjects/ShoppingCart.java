...
public class groupProject {
    public static void main(String[] args) {
        ...
        String myItem = sc.next();
        checkItemInStock(myItem, arr);
        dividerLineOfEquals();
    ...
    static void checkItemInStock(String item, String[] arr) {

        for (int i = 0; i < arr.length; i++) {
            if (arr[i].contains(item)) {
                System.out.println(item.toUpperCase() + "'s are in stock.");
            }
        }
            System.out.println("We currently don't have " + item.toUpperCase() + " in stock, sorry.");
        }
